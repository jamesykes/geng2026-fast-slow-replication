...
Float32 dot products are evaluated at explicit full-float32 precision. XLA lowers float32 `dot_general` to TF32 tensor cores by default on Ampere and newer NVIDIA hardware, which carries roughly `1e-3` relative accuracy. On an H100 that inflated the conditional-weight residual of `w(s,b|q)` from the float32 rounding scale near `1.2e-7` to about `4e-4`, above the reviewed `diagnostic_tolerance` of `1e-4`, while mass and symmetry stayed near `1e-7` and `1e-9`. The three contractions in `one_edge_moments_jax` are the only dot products in the pair-density calculation, and they form the conditional one-edge law and its payoff moments, so each passes `jax.lax.Precision.HIGHEST` explicitly. Storage stays float32, no formula, dtype, tolerance, projection or chronology changes, and the policy is not configurable: a pilot configuration cannot restore the platform default. The policy name enters the normalized configuration, the executable-configuration digest, the stage-invariant contract, the compile and invocation signatures, the doctor environment digest, and every stage artifact, so changing it invalidates incompatible prerequisites and compiled provenance.

One exact full-grid production experiment is authorised. It evolves the exact `G=131` separable pair density through `T=32` synchronous steps from the authoritative seeded histogram, matched against `R` independent complete ABM runs at `n=128` with `alpha=0.4` and `tau=2.0`. ABM run keys are folded in from the global run index rather than split by batch size, so a run's stream depends only on its index and is invariant to chunk size and restart boundaries; the sampling law itself is unchanged. Source times `0,1,2,4,8,16,24,31` pair each ABM record `t` with `P_t` before the update to `t+1`. Bin schemes are the existing coarse and fine plus one exact midpoint subdivision of every fine interval in both coordinates; the finest valid scheme is the primary comparison and coarser schemes and anchors are refinement diagnostics. Uncertainty is a complete-run clustered bootstrap and is reported as pointwise descriptive intervals; no multiplicity-adjusted hypothesis test is claimed, and pair theory does not itself supply the empirical cross-opponent covariance used by the hybrid estimand.

...
## Guarded Lambda Cloud GPU pilot

The next bounded milestone is operational preparation, not a GPU result. It adds exact CUDA 12/CUDA 13 JAX 0.7.2 environment specifications, a pre-import allocator bootstrap, a ctypes CUDA Driver API visible-ordinal mapper, a bounded GPU doctor, and an atomic prerequisite-driven pilot runner. The complete operator sequence is in [docs/LAMBDA_GPU_PILOT.md](docs/LAMBDA_GPU_PILOT.md).

The stages are doctor; flat/separable parity and timing at `G=3,5,9`; separable `G=17,33`; separable `G=65` with separately disabled `G=97`; disabled `G=131` analysis only; and a separately confirmed, hard one-step `G=131` execution. Exact compiled identity, complete live memory analysis, current stable-device-matched usable capacity, and scientific validity remain non-overridable. Device initialization inputs are constructed only after compiled and capacity admission; the combined executable constructs pair mass on device and never returns a full density to the host.

Local use is limited to CPU-unavailable doctor mode, strict config/resource dry runs, and mocked tests. No Lambda API, SSH, instance creation, full-grid lowering/compilation/execution, real GPU query, or GPU performance claim is part of local validation.

The pilot layer adds 31 CPU-only tests, all passing in both default and CPU+x64 modes. On 2026-08-20 the complete warnings-as-errors suite reported `346 passed, 8 skipped` by default and `354 passed` with CPU+x64 enabled. The doctor reported the expected unavailable GPU on the local ARM64 CPU host; every stage, including disabled full-grid templates, completed configuration/resource dry-run without lowering or compilation.
